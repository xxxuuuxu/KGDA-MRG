...
from promptmrg_classifier import load_promptmrg_classifier
import json
...
def build_tmodel(config, device):
    ...
    tmodel.load_state_dict(tcheckpoint['model'], strict=False)
    # missing, unexpected = tmodel.load_state_dict(tcheckpoint['model'], strict=False)
    tmodel.to(device)
    return tmodel
...
def main(config):
    ...
    if config.mode == "train":
        ...
        early_stopping = EarlyStopping(
            ...
            verbose=True,
            delta=config.delta,
            path=f"{config.dataset_name}_best_model.pt"
        )
        ...
        for epoch in range(config.start_epoch, config.epochs):
            ...
            early_stopping(validate_result['METEOR'], model)
    ...
    if config.mode == "test":
        if os.path.exists(config.test_path):
            ...
            weights_dict = torch.load(config.test_path, map_location='cpu')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--align_w', type=float, default=0.1)
