...
import copy
import os
from .visualize_utils import visualize_attention_heatmap
import numpy as np
import cv2
import shutil

@torch.no_grad()
def update_ema(model, tmodel, decay=0.999):

    for ema_param, param in zip(tmodel.parameters(), model.parameters()):
        if param.requires_grad:
            ema_param.data.mul_(decay).add_(param.data, alpha=1 - decay)

    for ema_buffer, buffer in zip(tmodel.buffers(), model.buffers()):
        ema_buffer.data.copy_(buffer.data)


def weak_alignment_loss(feat_s, feat_c):
    """
    feat_s: token-level (B, L, D)
    feat_c: class-feature-level (B, L, D)
    """
    # 1. mean-pooling as global representation
    gs = feat_s.mean(dim=1)  # (B, D)
    gc = feat_c.mean(dim=1)  # (B, D)

    # 2. cosine alignment (semantic)
    cos_loss = 1 - F.cosine_similarity(gs, gc, dim=-1).mean()

    # 3. projection consistency
    proj_s = F.normalize(gs, dim=-1)
    proj_c = F.normalize(gc, dim=-1)
    mse_loss = F.mse_loss(proj_s, proj_c)

    return cos_loss + 0.3 * mse_loss
...
def train_one_epoch(model, tmodel, class_model, criterion, criterionKD, data_loader,
    ...
    with tqdm.tqdm(total=total) as pbar:
        for images, masks, com_images, com_masks, caps, cap_masks, image_class in data_loader:
            ...
            t_outputs_global = tmodel(samples, caps[:, :-1], cap_masks[:, :-1], [thresholded_predictions, tokenizer])
            t_outputs_local = tmodel(com_samples, caps[:, :-1], cap_masks[:, :-1], [thresholded_predictions, tokenizer])
            outputs = model(com_samples, caps[:, :-1], cap_masks[:, :-1], [thresholded_predictions, tokenizer])

            final_class = model.last_class_feature
            hs = model.last_hs

            ce_loss = criterion(outputs.permute(0, 2, 1), caps[:, 1:])

            align_loss = weak_alignment_loss(hs, final_class)

            kd_loss_global = criterionKD(outputs, t_outputs_global.detach())
            kd_loss_local  = criterionKD(outputs, t_outputs_local.detach())

            kd_loss = config.delta * (alpha_teacher_weight * kd_loss_global +
                                      (1 - alpha_teacher_weight) * kd_loss_local)

            # loss = criterion(outputs.permute(0, 2, 1), caps[:, 1:]) + kd_loss
            loss = ce_loss + kd_loss + config.align_w * align_loss
            # loss = ce_loss + config.align_w * align_loss
            ...
            # global_step += 1
            # # update_ema_variables(model, global_model, ema_alpha, global_step)
            # update_ema_variables(model, local_model, ema_alpha, global_step)
